...
import { Check, ChevronDown, ChevronUp, Star, ShieldCheck, ArrowRight, Zap, Crown, Flame, QrCode } from 'lucide-react';
...
const DEFAULT_PLANS = [
  ...
  },
  {
    ...
    annualPrice: 49,
    badge: 'MOST POPULAR',
    description: 'The ultimate athletic package featuring fuel bar perks, group classes & guest passes.',
    popular: true,
    icon: Flame,
    features: [
      'Everything in Basic Plan',
      'InBody 770 Monthly Body Scans',
      'HYROX & High-Intensity Group Classes',
      '1 Free Monthly Fitness Coaching Consultation',
      '2 Guest Passes per Month',
      '15% Off Fuel Bar Protein Smoothies'
    ]
  },
  {
    id: 'vip-plan',
...
export default function MembershipsPage({ setActivePage }) {
  ...
  const handleSelectPlan = async (plan) => {
    if (!user) {
      showToast(`Selected ${plan.name}. Please complete registration to activate your QR pass.`, 'info');
      setActivePage('register');
      return;
    }

    try {
      const res = await renewMemberSubscription(user.id, plan.name);
      if (res.success) {
        setUser(prev => ({
          ...prev,
        ...
        showToast(`🎉 Subscription Activated: ${plan.name}! Present your QR Pass to Gym Admin.`, 'success');
